ActiveAdmin.register Delayed::Job, as: 'Job' do
  ...
  scope ("Failing Jobs") {|jobs| Delayed::Job.where('failed_at is not null')}
  scope ("Late Jobs") {|jobs| Delayed::Job.where('run_at <= ?', Time.now.getgm)}
  scope ("Scheduled Jobs") {|jobs| Delayed::Job.where('run_at > ?', Time.now.getgm)}
